...
build/texlive-%.txt: build/texlive-%.profile source/texmfrepo.txt
	...
	cd $(basename $@) && PATH=$(ROOT)/$(basename $@)/$(BINARCH_native):$$PATH TEXMFDIST=$(ROOT)/$(basename $@)/texmf-dist TEXMFVAR=$(ROOT)/$(basename $@)/texmf-dist/texmf-var TEXMFCNF=$(ROOT)/$(basename $@)/texmf-dist/web2c $(ROOT)/$(basename $@)/$(BINARCH_native)/fmtutil --all
	#  
